...
def test_main_ignores_blank_optional_field_in_style_rule(
    ...
    def fake_import_module(name):
        if name in ("styles.amber", "styles.fluorite"):
            imported_styles.append(name)
            return SimpleNamespace(style={"custom_style": {}, "rc": {}})
        return real_import_module(name)
...
def test_main_matches_any_style_rule_against_trade_fields(
    tmp_path, monkeypatch
):
    config = tg.configure("/tmp/not-used.ini", can_override=False)
    config["General"]["charts_directory"] = str(tmp_path)
    config["Styles"]["amber"] = repr(("any", "breakout"))
    journal = pd.DataFrame(
        {
            "Entry date": [pd.Timestamp("2024-01-02")],
            "Entry time": [pd.Timestamp("2024-01-02 09:00:00").time()],
    ...
    assert imported_styles == ["styles.amber"]


def test_main_skips_blank_style_rules_before_later_match(
    tmp_path, monkeypatch
):
    config = tg.configure("/tmp/not-used.ini", can_override=False)
    config["General"]["charts_directory"] = str(tmp_path)
    config["Styles"]["amber"] = repr(("order_specification", "short"))
    config["Styles"]["ametrine"] = repr(("", ""))
    config["Styles"]["fluorite"] = repr(("", ""))
    config["Styles"]["opal"] = repr(("order_specification", "long"))
    journal = pd.DataFrame(
        {
            "Entry date": [pd.Timestamp("2024-01-02")],
            "Entry time": [pd.Timestamp("2024-01-02 09:00:00").time()],
            "Symbol": ["1234"],
            "Order specification": ["long market order"],
            "Entry price": [100.0],
            "Exit time": [pd.Timestamp("2024-01-02 13:00:00").time()],
            "Exit price": [101.0],
        }
    ...
    def fake_import_module(name):
        if name in ("styles.fluorite", "styles.opal"):
            imported_styles.append(name)
            return SimpleNamespace(style={"custom_style": {}, "rc": {}})
        return real_import_module(name)
    ...
    assert imported_styles == ["styles.opal"]
...
def test_main_exits_cleanly_on_chart_render_failure(
    tmp_path, monkeypatch, capsys
):
    config = tg.configure("/tmp/not-used.ini", can_override=False)
    config["General"]["charts_directory"] = str(tmp_path)
    ...
    journal = pd.DataFrame(
        {
            ...
            "Order specification": ["long"],
            "Entry price": [100.0],
            "Exit time": [pd.Timestamp("2024-01-02 09:01:00").time()],
            "Exit price": [101.0],
        }
    )
    ...
    assert excinfo.value.code == 1
    captured = capsys.readouterr()
    assert "Unable to render chart from" in captured.out
...
def test_plot_charts_save_failure_preserves_existing_chart(
    tmp_path,
    monkeypatch,
):
    config = tg.configure("/tmp/not-used.ini", can_override=False)
    ...
    monkeypatch.setattr(
        tg.mpf,
        "plot",
        lambda *args, **kwargs: (FakeFigure(), [FakeAxes(), FakeAxes()]),
    )
    trade_data = {
        "entry_date": pd.Timestamp("2024-01-02"),
        "entry_time": pd.Timestamp("2024-01-02 09:00:00").time(),
    ...
    with pytest.raises(MarketDataError, match="Unable to write chart"):
        tg.plot_charts(
            config,
            trade_data,
            str(market_data_path),
    ...
    assert not list(tmp_path.glob(f".{chart_path.name}.*.tmp"))
